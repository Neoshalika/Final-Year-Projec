...
export class ProductService {
  ...
  async getAllBySubCat(id: number): Promise<Product[]>{
    let productDataPage = await this.http.get<Product[]>(ApiManager.getURL(`products/bysubcat/${id}`)).toPromise();
    productDataPage = productDataPage.map((product) => Object.assign(new Product(), product));
    return productDataPage;
  }
