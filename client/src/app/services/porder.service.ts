...
export class PorderService {
  ...
  async getAllBySupplier(id: number): Promise<Porder[]>{
    let porderDataPage = await this.http.get<Porder[]>(ApiManager.getURL(`porders/bysupplier/${id}`)).toPromise();
    porderDataPage = porderDataPage.map((porder) => Object.assign(new Porder(), porder));
    return porderDataPage;
  }
