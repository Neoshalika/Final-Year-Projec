...
export class PurchaseService {
  ...
  async getAllBySupplierForPayment(id: number): Promise<Purchase[]>{
    let purchaseDataPage = await this.http.get<Purchase[]>(ApiManager.getURL(`purchases/bysupplierforpayment/${id}`)).toPromise();
    purchaseDataPage = purchaseDataPage.map((purchase) => Object.assign(new Purchase(), purchase));
    return purchaseDataPage;
  }
