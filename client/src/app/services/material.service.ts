...
export class MaterialService {
  ...
  async getAllBasicActives(pageRequest: PageRequest): Promise<MaterialDataPage>{
    const url = pageRequest.getPageRequestURL('materials/basic/actives');
    const materialDataPage = await this.http.get<MaterialDataPage>(ApiManager.getURL(url)).toPromise();
    materialDataPage.content = materialDataPage.content.map((material) => Object.assign(new Material(), material));
    return materialDataPage;
  }

  async getAllBasicActivesBySupplier(supplierId: number): Promise<Material[]>{
    const materials = await this.http.get<Material[]>(ApiManager.getURL('materials/basic/actives/supplier/' + supplierId)).toPromise();
  ...
  async getPhoto(id: number): Promise<any>{
    return await this.http.get<any>(ApiManager.getURL(`materials/${id}/photo`)).toPromise();
  }
