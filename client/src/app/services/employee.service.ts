...
export class EmployeeService {
  ...
  async getAllByDesination(id: number): Promise<Employee[]>{
    let employeeDataPage = await this.http.get<Employee[]>(ApiManager.getURL(`employees/bydesignation/${id}`)).toPromise();
    employeeDataPage = employeeDataPage.map((employee) => Object.assign(new Employee(), employee));
    return employeeDataPage;
  }

  async getAllByDesinationForTaskAllocation(id: number): Promise<Employee[]>{
    let employeeDataPage = await this.http.get<Employee[]>(ApiManager.getURL(`employees/bydesignationfortaskallocation/${id}`)).toPromise();
    employeeDataPage = employeeDataPage.map((employee) => Object.assign(new Employee(), employee));
    return employeeDataPage;
  }
