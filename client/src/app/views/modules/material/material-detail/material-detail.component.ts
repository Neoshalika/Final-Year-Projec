...
export class MaterialDetailComponent extends AbstractComponent implements OnInit {
  ...
  async delete(): Promise<void>{
    const dialogRef = this.dialog.open(DeleteConfirmDialogComponent, {
      width: '300px',
      data: {message: this.material.materialsubcategory.name + '-' + this.material.name}
    });
    ...
    dialogRef.afterClosed().subscribe( async result => {
      ...
      try {
        await this.materialService.delete(this.selectedId);
  ...
  async loadData(): Promise<any> {
    ...
    if (this.material.photo){
      const photoOb = await this.materialService.getPhoto(this.selectedId);
