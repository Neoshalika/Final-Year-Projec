...
export class CustomerDetailComponent extends AbstractComponent implements OnInit {
  ...
  async delete(): Promise<void>{
    const dialogRef = this.dialog.open(DeleteConfirmDialogComponent, {
      width: '300px',
      data: {message: this.customer.code + '-' + this.customer.nametitle.name + ' ' + this.customer.name}
    });
  ...
  async loadData(): Promise<any> {
    ...
    this.order = await this.customerorderService.getAllByCustomer(this.selectedId);
  }
