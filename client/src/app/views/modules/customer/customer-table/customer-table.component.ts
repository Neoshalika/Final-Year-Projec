...
export class CustomerTableComponent extends AbstractComponent implements OnInit {
  ...
  primarycontactField = new FormControl();
  ...
  async loadData(): Promise<any> {
    ...
    pageRequest.addSearchCriteria('primarycontact', this.primarycontactField.value);
  ...
  setDisplayedColumns(): void{
    this.displayedColumns = ['code', 'nametitle', 'name', 'primarycontact', 'alternatecontact'];
  ...
  async delete(customer: Customer): Promise<void>{
    const dialogRef = this.dialog.open(DeleteConfirmDialogComponent, {
      width: '300px',
      data: {message: customer.code + '-' + customer.nametitle.name + ' ' + customer.name}
    });
