...
public class CustomerController{
    ...
    public Page<Customer> getAll(PageQuery pageQuery, HttpServletRequest request) {
        ...
        String primarycontact = pageQuery.getSearchParam("primarycontact");
        ...
        List<Customer> filteredCustomers = stream.filter(customer -> {
            ...
            if(primarycontact!=null)
                if(!customer.getPrimarycontact().toLowerCase().contains(primarycontact.toLowerCase())) return false;
            return true;
    ...
    public Page<Customer> getAllBasic(PageQuery pageQuery, HttpServletRequest request){
        accessControlManager.authorize(request, "No privilege to get all customers' basic data", UsecaseList.SHOW_ALL_CUSTOMERS, UsecaseList.ADD_CUSTOMERORDER, UsecaseList.UPDATE_CUSTOMERORDER);
    ...
    public Customer get(@PathVariable Integer id, HttpServletRequest request) {
        accessControlManager.authorize(request, "No privilege to get customer", UsecaseList.SHOW_CUSTOMER_DETAILS);
    ...
    public ResourceLink add(@RequestBody Customer customer, HttpServletRequest request) throws InterruptedException {
        ...
        if(customer.getFax() != null){
            Customer customerByFax = customerDao.findByFax(customer.getFax());
            if(customerByFax!=null) errorBag.add("fax","fax already exists");
        }
    ...
    public ResourceLink update(@PathVariable Integer id, @RequestBody Customer customer, HttpServletRequest request) {
        ...
        if(customer.getFax() != null){
            Customer customerByFax = customerDao.findByFax(customer.getFax());
            if(customerByFax!=null)
                if(!customerByFax.getId().equals(id))
