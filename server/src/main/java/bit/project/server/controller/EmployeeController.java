...
public class EmployeeController{
    ...
    public Page<Employee> getAll(PageQuery pageQuery, HttpServletRequest request) {
        ...
        List<Employee> filteredEmployees = stream.filter(employee -> {
            ...
            if(callingname!=null){
                String name = employee.getNametitle().getName()+" "+employee.getCallingname();
                if(!name.toLowerCase().contains(callingname.toLowerCase())) return false;
            }
    ...
    public Page<Employee> getAllBasic(PageQuery pageQuery, HttpServletRequest request){
        accessControlManager.authorize(request, "No privilege to get all employees' basic data", UsecaseList.SHOW_ALL_EMPLOYEES, UsecaseList.ADD_ADVANCEDPAYMENT, UsecaseList.UPDATE_ADVANCEDPAYMENT, UsecaseList.ADD_ATTENDANCE, UsecaseList.UPDATE_ATTENDANCE, UsecaseList.ADD_DELIVERY, UsecaseList.UPDATE_DELIVERY, UsecaseList.ADD_LOAN, UsecaseList.UPDATE_LOAN, UsecaseList.ADD_SALARY, UsecaseList.UPDATE_SALARY);
    ...
    public ResourceLink add(@RequestBody Employee employee, HttpServletRequest request) throws InterruptedException {
        ...
        if(employee.getEtfno() != null){
            Employee employeeByEtfno = employeeDao.findByEtfno(employee.getEtfno());
            if(employeeByEtfno!=null) errorBag.add("etfno","etfno already exists");
        }
    ...
    public ResourceLink update(@PathVariable Integer id, @RequestBody Employee employee, HttpServletRequest request) {
        ...
        if(employee.getEtfno() != null){
            Employee employeeByEtfno = employeeDao.findByEtfno(employee.getEtfno());
            if(employeeByEtfno!=null)
                if(!employeeByEtfno.getId().equals(id))
                    errorBag.add("etfno","etfno already exists");
        }
    ...
    public HashMap<String, String> getPhoto(@PathVariable Integer id, HttpServletRequest request){
        accessControlManager.authorize(request, "No privilege to get user photo", UsecaseList.SHOW_EMPLOYEE_DETAILS);
    ...
    @GetMapping("/bydesignation/{id}")
    public List<Employee> getAllByDesination(@PathVariable Integer id, PageQuery pageQuery, HttpServletRequest request){
        accessControlManager.authorize(request, "No privilege to get all employees' basic data", UsecaseList.SHOW_ALL_EMPLOYEES, UsecaseList.ADD_ADVANCEDPAYMENT, UsecaseList.UPDATE_ADVANCEDPAYMENT, UsecaseList.ADD_ATTENDANCE, UsecaseList.UPDATE_ATTENDANCE, UsecaseList.ADD_DELIVERY, UsecaseList.UPDATE_DELIVERY, UsecaseList.ADD_LOAN, UsecaseList.UPDATE_LOAN, UsecaseList.ADD_SALARY, UsecaseList.UPDATE_SALARY);
        return employeeDao.findAllByDesination(id);
    }

    @GetMapping("/bydesignationfortaskallocation/{id}")
    public List<Employee> getAllByDesinationForTaskAllocation(@PathVariable Integer id, PageQuery pageQuery, HttpServletRequest request){
        accessControlManager.authorize(request, "No privilege to get all employees' basic data", UsecaseList.SHOW_ALL_EMPLOYEES, UsecaseList.ADD_ADVANCEDPAYMENT, UsecaseList.UPDATE_ADVANCEDPAYMENT, UsecaseList.ADD_ATTENDANCE, UsecaseList.UPDATE_ATTENDANCE, UsecaseList.ADD_DELIVERY, UsecaseList.UPDATE_DELIVERY, UsecaseList.ADD_LOAN, UsecaseList.UPDATE_LOAN, UsecaseList.ADD_SALARY, UsecaseList.UPDATE_SALARY);
        List<Employee> employeeList = employeeDao.findAllByDesinationForTaskAllocation(id);
        ArrayList<Employee> employees = new ArrayList<>();
        List<Prorder> prorderList = prorderDao.findAll();
        if (prorderList != null){
            prorderList.forEach(prorder -> {
                if (prorder.getProrderemployeeList() != null){
                    prorder.getProrderemployeeList().forEach(prorderemployee -> {
                        if (prorderemployee.getEdate() == null){
                            System.out.println("a"+prorderemployee.getEmployee().getCallingname());
                            employees.add(prorderemployee.getEmployee());
                        }
                    });
                }
            });
