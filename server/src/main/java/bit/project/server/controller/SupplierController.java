...
public class SupplierController{
    ...
    public Page<Supplier> getAll(PageQuery pageQuery, HttpServletRequest request) {
        ...
        Integer categoryId = pageQuery.getSearchParamAsInteger("category");
        ...
        List<Supplier> filteredSuppliers = stream.filter(supplier -> {
            ...
            if(statusId!=null)
                if(!supplier.getSupplierstatus().getId().equals(statusId)) return false;
            if(categoryId!=null){
                boolean found = false;
                for (Materialcategory materialcategory :supplier.getMaterialcategoryList()) {
                    if(materialcategory.getId().equals(categoryId)){
                        found = true;
                        break;
                    }
                }
    ...
    public Page<Supplier> getAllBasic(PageQuery pageQuery, HttpServletRequest request){
        accessControlManager.authorize(request, "No privilege to get all suppliers' basic data", UsecaseList.SHOW_ALL_SUPPLIERS, UsecaseList.ADD_PORDER, UsecaseList.UPDATE_PORDER, UsecaseList.ADD_PURCHASE, UsecaseList.UPDATE_PURCHASE);
        return supplierDao.findAll(PageRequest.of(pageQuery.getPage(), pageQuery.getSize(), DEFAULT_SORT));
    }

    @GetMapping("/basic/actives")
    public Page<Supplier> getAllBasicActives(PageQuery pageQuery, HttpServletRequest request){
        accessControlManager.authorize(request, "No privilege to get all suppliers' basic data", UsecaseList.SHOW_ALL_SUPPLIERS, UsecaseList.ADD_PORDER, UsecaseList.UPDATE_PORDER, UsecaseList.ADD_PURCHASE, UsecaseList.UPDATE_PURCHASE);
        return supplierDao.findAllBasicActives(PageRequest.of(pageQuery.getPage(), pageQuery.getSize(), DEFAULT_SORT));
    }

    @GetMapping("/{id}")
    public Supplier get(@PathVariable Integer id, HttpServletRequest request) {
        accessControlManager.authorize(request, "No privilege to get supplier", UsecaseList.SHOW_SUPPLIER_DETAILS);
    ...
    public ResourceLink add(@RequestBody Supplier supplier, HttpServletRequest request) throws InterruptedException {
        ...
        if(supplier.getContact1() != null){
            ...
            if(supplierByContact1!=null) errorBag.add("contact1","Primary contact already exists");
        }
        ...
        if(supplier.getFax() != null){
            Supplier supplierByFax = supplierDao.findByFax(supplier.getFax());
            if(supplierByFax!=null) errorBag.add("fax","fax already exists");
        }
    ...
    public ResourceLink update(@PathVariable Integer id, @RequestBody Supplier supplier, HttpServletRequest request) {
        ...
        if(supplier.getContact1() != null){
            Supplier supplierByContact1 = supplierDao.findByContact1(supplier.getContact1());
            if(supplierByContact1!=null)
                if(!supplierByContact1.getId().equals(id))
                    errorBag.add("contact1","Primary contact already exists");
        }
        ...
        if(supplier.getFax() != null){
            Supplier supplierByFax = supplierDao.findByFax(supplier.getFax());
            if(supplierByFax!=null)
                if(!supplierByFax.getId().equals(id))
